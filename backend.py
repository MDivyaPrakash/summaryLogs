...
class summaryWriter:
    ...
    def query(self, payload):
        json = {
            "inputs": payload,
            "parameters": {"max_new_tokens": 512, "top_p": 0.95, "temperature": 0.75},
        }
    ...
    def get_response_wrapper(self, urls, question):
        ...
        url_list = (
            sorted([val.strip() for val in urls.split(",")])
            if isinstance(urls, str)
            else sorted([val.strip() for val in urls])
        )
        print(url_list)
